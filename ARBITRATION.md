...
- A **DAO DAO core** — the DAO passes a proposal whose msgs include `SubmitArbitration { question_id, winning_answer }`. When the proposal executes, the call lands on `cw-reality` with `info.sender == dao_core`. `cw-reality` checks `sender == question.arbitrator`, finalizes.
...
`SubmitArbitration { question_id, winning_answer }`:

- Loads the question. If `question.arbitrator.is_none()`, error.
- Checks `info.sender == question.arbitrator.unwrap()`. If not, error.
- Validates `winning_answer` is one of the answers already submitted for the question (the arbitrator picks among submitted answers; it does not author new ones).
- Marks the question as `Finalized` with `final_answer = winning_answer`. Triggers bond redistribution.

...
| Arbitrary arbitrator answer | The source permits the arbitrator to author any answer and payee. `ARBITRATION.md` still says it must choose a submitted answer. | Reconcile documentation. The market remains safe by recognizing only exact canonical bytes and mapping everything else to neutral. |
...
| Documentation drift | The contract README mentions first-class arbitration adapters while `ARBITRATION.md` rejects adapters; other notes describe surfaces not present in `msg.rs`. | Produce a source-versus-doc compatibility memo and fix documentation in a later, explicitly authorized phase. Market design must use the compiled schema and source behavior. |

...
**Scope:** issue #19 assurance that can land before factory issue #17 and LP issue #15.
...
The lifecycle suite exposed and now guards a reconciliation defect: after a position
redemption, `Solvency.principal_liability` is derived from the remaining terminal
numerator and unmatched neutral half, not the immutable principal-at-resolution
snapshot. This keeps forced excess visible after partial redemption.
...
- **#15 LP settlement:** `RedeemLp` and `ClaimLpAccrual` are currently
  `NotImplemented`. Therefore creator/LP redemption, partial LP burns, later dust,
  fee/slash ownership, and the full documented 105-JUNO terminal conservation vector
  remain #15-dependent. Issue #19 must not be closed until those landed paths are
  integrated.
...
Until #17 and #15 land, this work **Tracks #19** and does not satisfy all of #19's
acceptance criteria.
